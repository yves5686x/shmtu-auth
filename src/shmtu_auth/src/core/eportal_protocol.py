...
class EPortalClient:
    ...
    def _post_json(self, method: str, data: Dict, referer: str = "") -> Dict:
        ...
        try:
            ...
            logger.warning(f"pageInfo/login non-JSON response (status={res.status_code}): {text[:200]}")
            return {}
    ...
    def open_entry(self, query_string: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"Portal entry opened: status={res.status_code}, url={res.url}")
            return res.status_code == 200
        except Exception as e:
            logger.warning(f"Failed to open portal entry: {e}")
            return False
    ...
    def query_page_info(self, query_string: str) -> PortalPageInfo:
        ...
        logger.info(
            ...
            % (info.need_valid_code, info.password_encrypt, info.raw.get("service"))
        )
        return info
    ...
    def fetch_valid_code(self, valid_code_url: str) -> Optional[bytes]:
        ...
        try:
            ...
            if res.status_code == 200 and res.content:
                return res.content
            logger.warning(f"Failed to fetch validcode: status={res.status_code}")
        except Exception as e:
            logger.warning(f"Failed to fetch validcode: {e}")
        return None
    ...
    def query_services(self, query_string: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            logger.debug(f"getServices failed: {e}")
            return []
        ...
        if service_json and service_json not in ("[]", "null"):
            try:
                ...
                logger.debug(f"serviceJson parse failed: {e}")

        logger.info(f"Portal offered services: {services}")
        return services
    ...
    def query_account_service(self, query_string: str, username: str) -> str:
        ...
        **但登录流程不使用它做选择**：Docker / 容器里网络类型判断经常不准，
        所以调用方改成两种 service 都试（见 ``ShmtuNetAuthCore._portal_service_candidates``）。
        ...
        try:
            ...
            if text and text.lower() != "null":
                logger.info(f"Account bound service: {text}")
                return text
        except Exception as e:
            logger.debug(f"userV2.getServices failed: {e}")
        return ""
