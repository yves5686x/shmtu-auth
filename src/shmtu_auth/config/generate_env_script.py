...
from shmtu_auth.config.end_line import convert_to_crlf, convert_to_lf
...
def get_dict_depth(dictionary, current_depth=1) -> int:
    ...
    nested_depths = [get_dict_depth(value, current_depth + 1) for value in dictionary.values()]
    ...
    if dict_depth == 1:
        ...
        for key, value in data_dict.items():
            ...
            if default_mode:
                ...
                script_pwsh += f'CheckAndLoadEnvVariable `\n\t-EnvVariableName "{key}" `\n\t-DefaultValue "{value}"\n'
    else:
        # 深度大于 1 的字典，递归生成脚本
        for key, value in data_dict.items():
            str_spilt_line = "# " + "-" * 50
            ...
            if isinstance(value, dict):
                ...
                child_script_sh, child_script_pwsh = generate_env_script(child_dict, default_mode)
